...
# Be aware that bdtcoind and bdtcoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in bdtcoind.
#
...
# C locale.
#
# In contrast, bdtcoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where bdtcoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of bdtcoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in bdtcoind but
# "1,230000" in bdtcoin-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/bdtcoin-tx.cpp.*stoul"
    "src/bdtcoin-tx.cpp.*trim_right"
