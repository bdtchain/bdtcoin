...
def make_utxo(node, amount, confirmed=True, scriptPubKey=DUMMY_P2WPKH_SCRIPT):
    ...
    """
    fee = 1*COIN
    while node.getbalance() < jus_round((amount + fee)/COIN):
        node.generate(100)

    new_addr = node.getnewaddress()
    txid = node.sendtoaddress(new_addr, jus_round((amount+fee)/COIN))
...
class ReplaceByFeeTest(BdtcoinTestFramework):
    ...
    def test_doublespend_chain(self):
        ...
        # child fees - 40 BDTC - so this attempt is rejected.
        dbl_tx = CTransaction()
