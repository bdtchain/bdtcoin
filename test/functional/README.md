...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The Travis linter also checks this, but [possibly not in all cases](https://github.com/bdtcoin/bdtcoin/pull/14884#discussion_r239585126).
...
with the bdtcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a bdtcoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Bdtcoin Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the BdtcoinTestFramework
...
Taken from the [python-bdtcoinrpc repository](https://github.com/jgarzik/python-bdtcoinrpc).
...
Test objects for interacting with a bdtcoind node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-bdtcoinlib)
