...
We can import the `TestShell` by adding the path of the configured Bdtcoin Core
...
```
>>> import sys
>>> sys.path.insert(0, "/path/to/bdtcoin/build/test/functional")
...
* `/path/to/bdtcoin/build/test/functional/combine_logs.py
...
|---|---|---|
| `bind_to_localhost_only` | `True` | Binds bdtcoind P2P services to `127.0.0.1` if set to `True`.|
| `cachedir` | `"/path/to/bdtcoin/build/test/cache"` | Sets the bdtcoind datadir directory. |
| `chain`  | `"regtest"` | Sets the chain-type for the underlying test bdtcoind processes. |
| `configfile` | `"/path/to/bdtcoin/build/test/config.ini"` | Sets the location of the test framework config file. |
...
| `rpc_timeout` | `60` | Sets the RPC server timeout for the underlying bdtcoind processes. |
