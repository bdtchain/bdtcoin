...
// Copyright (c) 2009-present The Bdtcoin Core developers
...
static RPCHelpMan generatetodescriptor()
{
    return RPCHelpMan{
        ...
        "Mine to a specified descriptor and return the block hashes.",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated bdtcoin to."},
        ...
        "Mine to a specified address and return the block hashes.",
         {
             ...
             {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated bdtcoin to."},
             {"maxtries", RPCArg::Type::NUM, RPCArg::Default{DEFAULT_MAX_TRIES}, "How many iterations to try."},
         },
         RPCResult{
             RPCResult::Type::ARR, "", "hashes of blocks generated",
         ...
         RPCExamples{
            ...
            + "If you are using the " CLIENT_NAME " wallet, you can get a new address to send the newly generated bdtcoin to with:\n"
...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated bdtcoin to."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Expose signet challenge when:
    // - on a true signet chain, or
    // - a challenge is configured and the next height is at or past auth_activation_height
    const auto& consensus = chainman.GetConsensus();
    const bool signet_active_info =
        chainman.GetParams().GetChainType() == ChainType::SIGNET ||
        (!consensus.signet_challenge.empty() && next_index.nHeight >= consensus.auth_activation_height);
    if (signet_active_info) {
        obj.pushKV("signet_challenge", HexStr(consensus.signet_challenge));
    }
...
static RPCHelpMan getblocktemplate()
{
    return RPCHelpMan{"getblocktemplate",
        ...
        "    https://github.com/bdtcoin/bips/blob/master/bip-0022.mediawiki\n"
        "    https://github.com/bdtcoin/bips/blob/master/bip-0023.mediawiki\n"
        "    https://github.com/bdtcoin/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
        "    https://github.com/bdtcoin/bips/blob/master/bip-0145.mediawiki\n",
        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            // Explicitly enforce signet solution in proposal mode so that
            // signet debug logs and validation are exercised here as well,
            // but only once authorization is activated by height.
            ...
            if (pprev) {
                const int next_height = pprev->nHeight + 1;
                const bool signet_active = !consensus_params.signet_challenge.empty() && next_height >= consensus_params.auth_activation_height;
                if (signet_active) {
                    if (!CheckSignetBlockSolution(block, consensus_params)) {
                        return "bad-signet-solution";
                    }
                }
            }
    ...
    // Determine whether signet-style authorization is active for the next block
    const bool signet_active = consensusParams.signet_blocks ||
        (!consensusParams.signet_challenge.empty() && (pindexPrev->nHeight + 1) >= consensusParams.auth_activation_height);

    // If active, the client must opt-in with the 'signet' rule
    if (signet_active && setClientRules.count("signet") != 1) {
    ...
    // Expose signet_challenge only when authorization is active for the next block
    if (signet_active) {
...
static RPCHelpMan submitblock()
{
    ...
    return RPCHelpMan{"submitblock",
        ...
        "See https://en.bdtcoin.it/wiki/BIP_0022 for full specification.\n",
        {
