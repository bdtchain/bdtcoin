...
{
    fs::path settings = GetPathArg("-settings", BDTCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Bdtcoin
    //   new: C:\Users\Username\AppData\Local\Bdtcoin
    // macOS: ~/Library/Application Support/Bdtcoin
    // Unix-like: ~/.bdtcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Bdtcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Bdtcoin";
#else
    fs::path pathRet;
