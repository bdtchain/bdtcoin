This directory contains the source code for the Bdtcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Bdtcoin UI.
...
- ClientModel is used by the main application `bdtcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bdtcoin:... link from a browser).
...
* UI elements like BdtcoinAmountField, which inherit from QWidget.
* `bdtcoinstrings.cpp`: automatically generated
* `bdtcoinunits.(h/cpp)`: BDTC / mBDTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bdtcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bdtcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bdtcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `bdtcoin-qt`
9. You're all set! Start developing, building, and debugging the Bdtcoin Core GUI
