...
#include <qt/bdtcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bdtcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BdtcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new BdtcoinAddressCheckValidator(parent));
}

bool parseBdtcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bdtcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("bdtcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BdtcoinUnits::parse(BdtcoinUnits::BDTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBdtcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBdtcoinURI(uriInstance, out);
}

QString formatBdtcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bdtcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BdtcoinUnits::format(BdtcoinUnits::BDTC, info.amount, false, BdtcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBdtcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BDTCOIN_CONF_FILENAME));
    ...
    /* Open bdtcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bdtcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bdtcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Bdtcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Bdtcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bdtcoin.desktop";
    return GetAutostartDir() / strprintf("bdtcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bdtcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Bdtcoin\n";
        else
            optionFile << strprintf("Name=Bdtcoin (%s)\n", chain);
