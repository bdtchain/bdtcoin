...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BdtcoinUnit>("BdtcoinUnit");
#else
    qRegisterMetaType<BdtcoinUnit>("BdtcoinUnit");
#endif
}
...
bool BdtcoinApplication::createOptionsModel(bool resetSettings)
{
...
void BdtcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    ...
    connect(window, &BdtcoinGUI::quitRequested, this, &BdtcoinApplication::requestShutdown);
...
void BdtcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
void BdtcoinApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BdtcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BdtcoinApplication::handleRunawayException);
    connect(this, &BdtcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BdtcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
void BdtcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
...
void BdtcoinApplication::handleNonFatalException(const QString& message)
{
...
bool BdtcoinApplication::event(QEvent* e)
{
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BDTCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bdtcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bdtcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bdtcoin.conf, determine network, switch to network specific
