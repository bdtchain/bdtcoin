...
/** Call node.updateRwSetting() with Bdtcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Bdtcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bdtchain/bdtcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bdtcoin 22.x will not try to read these.
...
//! Convert enabled/size values to bdtcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from bdtcoin -prune setting.
...
//! Get pruning size value to show in GUI from bdtcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBdtcoinUnit")) {
        settings.setValue("DisplayBdtcoinUnit", QVariant::fromValue(BdtcoinUnit::BDTC));
    }
    QVariant unit = settings.value("DisplayBdtcoinUnit");
    if (unit.canConvert<BdtcoinUnit>()) {
        m_display_bdtcoin_unit = unit.value<BdtcoinUnit>();
    } else {
        m_display_bdtcoin_unit = BdtcoinUnit::BDTC;
        settings.setValue("DisplayBdtcoinUnit", QVariant::fromValue(m_display_bdtcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bdtcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bdtcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BdtcoinUnit>() == m_display_bdtcoin_unit) return;
    m_display_bdtcoin_unit = new_unit.value<BdtcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBdtcoinUnit", QVariant::fromValue(m_display_bdtcoin_unit));
    Q_EMIT displayUnitChanged(m_display_bdtcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bdtchain/bdtcoin/pull/8273
    ...
    // (https://github.com/bdtcoin-core/gui/issues/567).
