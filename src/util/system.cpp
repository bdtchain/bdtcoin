...
const char * const BDTCOIN_CONF_FILENAME = "bdtcoin.conf";
const char * const BDTCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bdtchain/bdtcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bdtcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BDTCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bdtcoin
    // macOS: ~/Library/Application Support/Bdtcoin
    // Unix-like: ~/.bdtcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bdtcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bdtcoin";
#else
    // Unix-like
    return pathRet / ".bdtcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BDTCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Bdtcoin Core copyright is not removed by accident
    if (copyright_devs.find("Bdtcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bdtcoin Core developers";
    }
