 ...
 *
 * See https://bdtcointalk.org/index.php?topic=8392.msg127623#msg127623
 *
...
bool static IsLowDERSignature(const valtype &vchSig, ScriptError* serror) {
    ...
    // https://bdtcoin.stackexchange.com/a/12556:
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    LogDebug(BCLog::VALIDATION,
             "VerifyScriptDbg: flags=%u scriptSig_len=%u witness_items=%u spk=%s\n",
             flags,
             (unsigned)scriptSig.size(),
             (unsigned)witness->stack.size(),
             HexStr(scriptPubKey));
    ...
    if (flags & SCRIPT_VERIFY_WITNESS) {
        if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
            LogDebug(BCLog::VALIDATION,
                     "VerifyScriptDbg: witness_program version=%d prog_len=%u\n",
                     witnessversion,
                     (unsigned)witnessprogram.size());
            hadWitness = true;
            if (scriptSig.size() != 0) {
