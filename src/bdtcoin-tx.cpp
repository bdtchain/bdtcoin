...
static void SetupBdtcoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bdtcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bdtcoin-tx tool is used for creating and modifying bdtcoin transactions.\n\n"
                "bdtcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bdtcoin transaction, or with \"-create [commands]\" to create a hex-encoded bdtcoin transaction.\n"
                "\n"
                "Usage: bdtcoin-tx [options] <hex-tx> [commands]\n"
                "or:    bdtcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bdtcoin transaction
