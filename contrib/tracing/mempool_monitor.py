...
""" Example logging Bdtcoin Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into bdtcoind with pid {pid}")
    bdtcoind_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bdtcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bdtcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bdtcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bdtcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bdtcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of bdtcoind>")
        exit(1)
