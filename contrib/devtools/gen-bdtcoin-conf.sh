...
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/bdtcoin.conf}

[ ! -x "$BDTCOIND" ] && echo "$BDTCOIND not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($BDTCOIND --version)
if [[ $VERSION_OUTPUT == *"dirty"* ]]; then
  DIRTY="${DIRTY}${BDTCOIND}\n"
fi

if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $BDTCOIND was built from a dirty tree.\n"
  echo -e "To safely generate a bdtcoin.conf file, please commit your changes to $BDTCOIND, rebuild, then run this script again.\n"
fi

echo 'Generating example bdtcoin.conf file in share/examples/'
...
## Generated by contrib/devtools/gen-bdtcoin-conf.sh.
##
...
# parse the output from bdtcoind --help
