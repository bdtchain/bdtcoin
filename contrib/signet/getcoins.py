...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bdtcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bdtcoin-cli', help='bdtcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bdtcoin address to which the faucet should send')
...
parser.add_argument('bdtcoin_cli_args', nargs='*', help='Arguments to pass on to bdtcoin-cli (default: -signet)')

args = parser.parse_args()

if args.bdtcoin_cli_args == []:
    args.bdtcoin_cli_args = ['-signet']


def bdtcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bdtcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bdtcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bdtcoin_cli(['getnewaddress', 'faucet', 'bech32'])
