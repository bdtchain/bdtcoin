...
The Bdtcoin Core daemon can be configured to act as a trusted "border
router", implementing the bdtcoin wire protocol and relay, making
...
The ZeroMQ feature in Bdtcoin Core requires the ZeroMQ API >= 4.0.0
...
introduced into bdtcoind directly. Furthermore, no information is
...
using. Bdtcoind appends an up-counting sequence number to each
