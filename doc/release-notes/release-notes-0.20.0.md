...
- Instead of running `make` simply, you should instead run
  `BDTCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Bdtcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Bdtcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Bdtcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Bdtcoin Core. Bdtcoin Core logs debug messages
...
- OpenSSL is no longer used by Bdtcoin Core.  (#17265)

- BIP70 support has been fully removed from Bdtcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bdtchain/bdtcoin/issues/16599),
  [PR #16702](https://github.com/bdtchain/bdtcoin/pull/16702), and the
  `bdtcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Bdtcoin Core starts are now
...
- The "Start Bdtcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Bdtcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bdtcoin-core/HWI) for
  signing.  Future versions of Bdtcoin Core should support a GUI option
...
- The `bdtcoin-cli` utility used with the `-getinfo` parameter now
...
- The `bdtcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bdtcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bdtcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBDTCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bdtcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBDTCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bdtcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bdtcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bdtcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bdtcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bdtcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bdtcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Bdtcoin Core files (hebasto)
...
- #17119 Fix broken bdtcoin-cli examples (andrewtoth)
...
- #17648 Rename wallet-tool references to bdtcoin-wallet (hel-o)
...
- #18341 Replace remaining literal BDTC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bdtcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bdtcoin-cli -getinfo (malevolent)
