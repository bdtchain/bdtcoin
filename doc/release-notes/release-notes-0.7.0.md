Bdtcoin version 0.7.0 is now available for download at:
  http://sourceforge.net/projects/bdtcoin/files/Bdtcoin/bdtcoin-0.7.0/

We recommend that everybody running prior versions of bdtcoind/Bdtcoin-Qt
...
in your terminal, then install the bdtcoin-qt package:
  sudo apt-get install bdtcoin-qt
...
Bdtcoin Improvement Proposals implemented
...
Core bdtcoin handling and blockchain database
...
* Ensure a changed bdtcoin unit immediately updates all GUI elements that use units
...
* (Windows only): enable ASLR and DEP for bdtcoin-qt.exe
* (Windows only): add meta-data to bdtcoin-qt.exe (e.g. description)
...
* Bash programmable completion for bdtcoind(1)
