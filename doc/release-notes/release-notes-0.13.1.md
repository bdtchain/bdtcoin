Bdtcoin Core version 0.13.1 is now available from:

  <https://bdtcoin.org/bin/bdtcoin-core-0.13.1/>
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bdtcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bdtcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bdtcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of bdtcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of bdtcoins being spent is exactly
...
- **Increased security for multisig:** Bdtcoin addresses (both P2PKH addresses
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Bdtcoin's current ledger (called the UTXO set),
  ...
  note that Bdtcoin Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bdtcoin users to individually opt-in to almost any change in the Bdtcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bdtcoin Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Bdtcoin community members are studying several other improvements
...
Since Bdtcoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/bdtcoin/bips/blob/master/bip-0147.mediawiki
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in bdtcoinconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.bdtcoinstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libbdtcoinconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_bdtcoin by itself (luke-jr)
...
- #8829 `2a8bca4` Add bdtcoin-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` bdtcoin-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_bdtcoinds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name bdtcoin.qrc  (bdtcoinsSG)
...
- #8817 `bcf3806` update bdtcoin-tx to output witness data (jnewbery)
