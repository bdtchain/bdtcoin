...
Bdtcoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Bdtcoin Core on unsupported systems.
...
to open the Bdtcoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Bdtcoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bdtchain/bdtcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Bdtcoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Bdtcoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bdtcoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Bdtcoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Bdtcoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Bdtcoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Bdtcoin Core,
  ...
  This prevents Bdtcoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Bdtcoin Core and nodes that have manually enabled BIP37 support,
...
- The `testnet` field in `bdtcoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Bdtcoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Bdtcoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Bdtcoin Core will be bound to
  the standard port 7393 even if a different port is configured for
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Bdtcoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Bdtcoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Bdtcoin AEAD (jonasschnelli)
...
- #15639 bdtcoin-wallet tool: Drop libbdtcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as BDTC/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in bdtcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bdtcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bdtcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from bdtcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bdtcoin to auto generated project files (sipsorcery)
...
- #15773 Add BdtcoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bdtcoin output (practicalswift)
...
- #15896 feature_filelock, interface_bdtcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Bdtcoin Core (luke-jr)
...
- #16277 Suppress output in test_bdtcoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as BDTC/kB (jonatack)
...
- #16489 log: harmonize bdtcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bdtcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bdtcoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBBDTCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update bdtcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bdtcoin.conf (fanquake)
...
- #16621 add default bdtcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bdtcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bdtcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bdtcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bdtcoin/bdtcoin/).
