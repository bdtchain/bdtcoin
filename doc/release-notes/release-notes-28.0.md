Bdtcoin Core version 28.0 is now available from:

  <https://bdtcoincore.org/bin/bdtcoin-core-28.0/>
...
Running Bdtcoin Core binaries on macOS requires self signing.
```
cd /path/to/bdtcoin-28.0/bin
xattr -d com.apple.quarantine bdtcoin-cli bdtcoin-qt bdtcoin-tx bdtcoin-util bdtcoin-wallet bdtcoind test_bdtcoin
codesign -s - bdtcoin-cli bdtcoin-qt bdtcoin-tx bdtcoin-util bdtcoin-wallet bdtcoind test_bdtcoin
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Bdtcoin
...
Support for Testnet4 as specified in [BIP94](https://github.com/bdtcoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Bdtcoin`
to `C:\Users\Username\AppData\Local\Bdtcoin`. Bdtcoin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/bdtchain/bdtcoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libbdtcoinconsensus Removal
---------------------------

The libbdtcoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Bdtcoin Core was listening for P2P connections, either using
...
- Bdtcoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/bdtcoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Bdtcoin Core with the configuration
...
are now required to compile Bdtcoin Core. (#29091, #30263)

- The minimum required glibc to run Bdtcoin Core is now
...
- A new `-testdatadir` option has been added to `test_bdtcoin` to allow specifying the
...
Previous releases of Bdtcoin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
