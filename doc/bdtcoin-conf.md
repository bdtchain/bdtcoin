...
Changes to the configuration file while `bdtcoind` or `bdtcoin-qt` is running only take effect after restarting.
...
The `settings.json` file contains dynamic settings that are set by the Bdtcoin Core GUI and RPCs at runtime, and augment or replace the static settings specified in the `bdtcoin.conf` file.

Command line options also augment or replace `bdtcoin.conf` options, and can be useful for scripting and debugging.

It is possible to see which setting values are in use by checking `debug.log` output. Any unrecognized options that are found in `bdtcoin.conf` also show up as warnings in `debug.log` output.
...
-- | -- | --
Windows | `%LOCALAPPDATA%\Bdtcoin\` | `C:\Users\username\AppData\Local\Bdtcoin\bdtcoin.conf`
...
An example configuration file can be generated by [contrib/devtools/gen-bdtcoin-conf.sh](../contrib/devtools/gen-bdtcoin-conf.sh).
Run this script after compiling to generate an up-to-date configuration file.
The output is placed under `share/examples/bdtcoin.conf`.
To use the generated configuration file, copy the example file into your data directory and edit it there, like so:

```
# example copy command for linux user
cp share/examples/bdtcoin.conf ~/.bdtcoin
```
