# CJDNS support in Bdtcoin Core

It is possible to run Bdtcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bdtcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bdtcoin network and individual nodes.
...
before it will work with your Bdtcoin Core node. This is described in steps
...
## Run Bdtcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Bdtcoin Core
...
When enabled, this option tells Bdtcoin Core that it is running in an
...
IPv6 local network. This helps Bdtcoin Core perform better address management:
...
CJDNS support was added to Bdtcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bdtcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bdtchain/bdtcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Bdtcoin Core

There are several ways to see your CJDNS address in Bdtcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bdtcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bdtcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bdtcoin-cli help getnodeaddresses` for details.
